...
class AgentTeam:
    ...
    def build_team(self) -> None:
        ...
        for agent in self.AGENTS:
            model_deployment_name = agent.get("model_deployment_name")
            if model_deployment_name is None:
                raise ValueError("Model deployment name must be specified for each agent.")
            name = agent.get("name")
                ...
                if isinstance(func, str):
                    try:
                        ...
                        if callable(func_ref):
                            agent_functions.add(func_ref)
                        else:
                            raise ValueError(f"Function '{func}' is not callable.")
                    except KeyError:
                        raise ValueError(f"Function '{func}' not found in global scope.")
                else:
                    raise ValueError("Function must be specified as a string name.")
            if agent_functions:
                # Ensure agent_functions is a set of callable functions
                if not isinstance(agent_functions, set):
                    agent_functions = set(agent_functions)
                functions = FunctionTool(functions=agent_functions)
                toolset.add(functions)
                all_functions.update(agent_functions)
            self.add_agent(
